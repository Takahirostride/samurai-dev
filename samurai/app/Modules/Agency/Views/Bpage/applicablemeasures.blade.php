				...
				<div class="row">
					<div class="col-sm-12">
						<ul class="tab-profile nav nav-tabs">
							...
							<li class="tab-style-grey active"><a href="{{route('agency.ApplicableMeasures',['policy_id' => $policy_id, 'user_id' => $user->id])}}">対応可能施策</a></li>
						</ul>
					</div>
				</div>
				<div class="row">
					<div class="col-sm-12">
						...
						<div class="dreview-row">
							<div class="row">
								<div class="col-xs-2">
									{{ HTML::image($post->policy['image_id']) }}
								</div>
								<div class="col-xs-10">
									<div class="dreview-item-right">
										<div class="dreview-item-left-box dselect4">
											<h4 class="dreview-item-head">{{str_limit($post->name, 38)}}</h4>
											<div class="clearfix"></div>
											<a href="{{route('subsidydetail', ['id'=>$post->policy['id']])}}" class="dreview-text dthead"><strong>登録期関:</strong>{{str_limit($post->policy['register_insti_detail'], 40)}} <strong> / 募集時期:</strong>{{str_limit($post->policy['subscript_duration'], 40)}} </a><!--<strong> 金額:</strong> 取得可能金額:100万円</p> -->
											<p class="dreview-text">{{str_limit($post->target, 192)}}</p>
											{!! Button::getFavourButtons($post->policy['id']) !!}
				                            @if($post->acquire_budget_display)
				                            <div class="btn-cost2">
				                                <label class="label-cost">{{$post->acquire_budget_display}}</label>
				                            </div>
				                            @endif
										</div>
										...
										<div class="dreview-item-right-box dselect4">
											<div class="dreview-list-box">
												...
												<div class="dreview-list-item">
													<p>業務 : {{$post->document_business_price}}</p>
												</div> <!-- end .dreview-list-item -->
												<div class="dreview-list-item">
													<p>提案 : {{$post->request_count}}</p>
												</div> <!-- end .dreview-list-item -->
												<div class="dreview-list-item">
													<p>残り : {{$post->remaining}}</p>
												</div> <!-- end .dreview-list-item -->
											</div>
										</div>
