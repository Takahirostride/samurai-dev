						...
						<table class="table table-bordered table-hover table-myjob">
						    ...
						    <tbody>
							    ...
							    <tr>
							        ...
							        <td class="td-link"><a href="{{URL::route( ($jobType==1)?'agency.jobs.rq_detail':'agency.jobs.detail', [$msg->hire_id] )}}">{{$msg->policy_name}}</a></td>
							        <td class="td-link"><a onclick="return false;" href="{{URL::route('agency.jobs.com_detail', [$msg->hire_id] )}}">{{$msg->displayname}}</a></td>
							        <td class="td-link"><a href="{{URL::route(($jobType==1)?'agency.jobs.rq_bid_job':'agency.jobs.bid_job', [$msg->hire_id] )}}"> @if($msgNote[$key]['unread_message']<=0)依頼送信済み@else 返信あり @endif </a></td>
							    </tr>
							    @endforeach
				...
				<div class="row">
					<div class="col-sm-12 subsidydetail">
						...
						<div class="middle-boxcol2">
							{!! Button::getFavourButtons($result->id) !!}
