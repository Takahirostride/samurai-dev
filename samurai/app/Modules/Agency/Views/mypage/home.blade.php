	...
	<div class="container">
		...
		<div class="row">
		        ...
		        <div class="box-noti-list">
		        	...
		        	<table class="table table-bordered table-home-notify">
			        	...
			        	<tr>
			        		...
			        		<td><a href="{{URL::route('agency.notify', [$nof->id] )}}">{{$nof->title}}</a></td>
			        	</tr>
			        	@endforeach
		        	</table>
		        	
		        	<p class="text-right "><a href="{{URL::route('agency.notifylist')}}" class="link-right">すべてのお知らせを見る</a></p>
		        </div>
		        <div class="box-2">
		        	<h2 class="page-title">お勧めの案件</h2>
		        	@foreach ($homePolicy as $key => $item)
						{!! Button::showPolicyList($item, 'agency.bselect') !!}
					@endforeach
			...
			<div class="col-sm-2 sidebar-right">
				@include('Agency::includes.sidebar-right-home')
