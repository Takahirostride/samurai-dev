						...
						<table class="table table-bordered table-hover table-myjob">
						    ...
						    <tbody>
							    ...
							    <tr>
							        ...
							        <td class="td-link"><a href="{{URL::route('agency.jobs.rq_detail', [$msg->hire_id] )}}">{{$msg->policy_name}}</a></td>
							        <td class="td-link"><a onclick="return false;" href="{{URL::route('agency.jobs.rq_com_detail', [$msg->hire_id] )}}">{{$msg->displayname}}</a></td>
							        <td class="td-link"><a href="{{URL::route('agency.jobs.rq_bid_job', [$msg->hire_id] )}}"> @if($msgNote[$key]['unread_message']<=0)依頼送信済み@else 返信あり @endif </a></td>
							    </tr>
							    @endforeach
