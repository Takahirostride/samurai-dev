@php
    ...
    $is_register = empty($element->is_register) ? false:true;
    $is_add_mail = empty($element->email) ? false : true;
@endphp
<tr class="row-link" data-link="{{ route('asp_status_client_document',$element) }}">
    <td>{{ $element->name }}</td>
    ...
    <td class="noLink">
        <a href="{{ route('asp_manage_clients_register_destroy',['id'=>$element->id]) }}" class="btn btn-danger destroy-company" data-name="{{ $element->name }}" >顧客削除</a>
    </td>
</tr> 