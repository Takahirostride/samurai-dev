								...
								<div class="subsidy-list-item">
									...
									<div class="row">
										...
										<div class="subsidy-list-col2">
											@if($value['count_agency'] > 0)
											<div class="subsidylist-item-av div-style-grey">
												<img src="{{URL::to('/')}}/{{$value['rand_agency']->image}}" alt="">
												<div class="itemav-info">
													<p>実績：{{$value['rand_agency']->result}}件</p>
													@if($value['rand_agency']->evaluate)
													<p>評価：なし</p>
													@endif
													<div class="star-rating">
														{!! Button::getRating($value['rand_agency']->evaluate) !!}
												</div> <!-- end .itemav-info -->
												@if($value['rand_agency']->evaluate >=0)
										...
										<div class="subsidy-list-col3">
											<p>対応可能士業</p>
											<a class="imagecenter" href="{{URL::route('client.fselect',$value['id'])}}/?searchtype=0">
                                                <img src="{{URL::to('/')}}/assets/common/images/avatar.png" style="max-width:50px;max-height:70px;">
                                            </a>
                                            <a>{{$value['count_agency']}}人</a>
											@if($value['seller_exist_flag']=='success')
											<p>申請希望企業</p>
...
@section('script')
	<script>
		...
		function search(){
			...
			var url = 'hide?order='+option+'&per_page='+num_page;
                window.location=url;     
		}
	</script>
@endsection