				...
				<div class="row">
					<div class="col-sm-12">
						<ul class="nav nav-tabs tablinksub bordernone">
						    <li >
						        <a href="{{URL::route('client.message')}}">すべて</a>
						    </li>
						    <li class="active">
						        <a href="{{URL::route('client.message_per_case')}}">案件ごと</a>
						    </li>
						    <li>
						        <a href="{{URL::route('client.message_every_massey')}}">士業ごと</a>
						    </li>
						    <li>
						        <a href="{{URL::route('client.message_display_unread_only')}}">未読のみ表示</a>
						    </li>
						</ul><!-- end .tablinksub -->
						<div class="row" style="margin-top:10px;" ng-if="toptabflag==2">
			                <h5 class="text-primary" style="display:initial;float:left;"><b>案件名 : </b></h5>
			                <div class="col-sm-7">
			                	<div class="angularsl" id="policy_change" >
			                		<select id="policy">
			                		@if($policies)
						...
						<div class="row">
							...
							<div class="col-sm-9 pdl-0">
								...
								{{ Form::open(array('url' => '/client/mypage/send_message', 'method' =>'POST','id'=>'form','enctype'=>'multipart/form-data')) }}
								<table class="table table-bordered mgbt-0">
									<tbody>
										<tr>
											<td>
...
@section('script')
	<script>
		...
		$(document).on('click','.tr_click', function(){
			...
			console.log(hire_id);
			$.ajax({
                url: '/client/mypage/getmessagebyid?user_id='+user_id+'&hire_id='+hire_id,
                dataType: 'JSON',
		...
		$(document).ready(function(){
			$(document).on('click', '#policy_change .showoption div', function(){
		        var parent_id = $(this).parent().parent().attr('id');
		        var policy_id = $('#'+parent_id).find('select').val();
		        var key = $('#search').val();
		        //console.log(key);
		        search(policy_id,key);
			...
			function search(policy_id,key){

				html_mes_table = "";
				$.ajax({
	                url: '/client/mypage/searchmessagepercase?policy_id='+policy_id+'&keyword='+key,
	                dataType: 'JSON',
	                success: function(json) {
        ...
        function downloaddocfile(filepath,filename){
            window.open("/client/mypage/downloadfile/"+filepath+"/"+filename);
        }
	    ...
	    function sendmessage(){
	        ...
	        $.ajax({
	            url: '/client/mypage/send_message',
	            type: 'POST',
	            data: fd,
	            async: false,
