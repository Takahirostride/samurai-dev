							...
							<div class="col-sm-9 pdl-0">
								...
								<table class="table table-bordered mgbt-0">
									<tbody>
										<tr>
											<td>
												<h5 class="text-primary" id="policy_name_update"><b>@if(isset($hire_data[0])){{$hire_data[0]->policy_name}}（案件名）@endif</b></h5>
											</td>
										</tr>
										<tr>
											<td class="paddingnone">
...
@section('script')
	<script>
		...
		$(document).on('click','.tr_click', function(){
			...
			console.log(hire_id);
			$.ajax({
                url: '/client/mypage/getmessage_display_unread_only_byid?user_id='+user_id+'&hire_id='+hire_id,
                dataType: 'JSON',
		...
		$('#search').keyup(function(){
			...
			html_mes_table = "";
			$.ajax({
                url: '/client/mypage/searchmessage_display_unread_only?keyword='+key,
                dataType: 'JSON',
                success: function(json) {
