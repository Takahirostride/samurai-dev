	...
	<div class="col-sm-12">
			...
			<div class="profile-reg-form text-center">
			    ...
			    <dl class="row">
			        ...
			        <dd class="col-sm-8">
			        	<ul class="lst-col2 lst-trades">
			        			@php  
								$trades = App\Modules\Admin\Models\Trade::listAllDisplay();
			        			@endphp
			        			@foreach($trades as $ii=>$trade)
			        			<li>
			        					<div class="checkbox no-mgr">
			        						<label>
			        							<input type="checkbox" name="user_business[]" value="{{$ii}}" class="cbcheck" 
			        							@if($user->trade->contains('id',$ii)) checked="checked" @endif>
			        							{{$trade}}
			        						</label>
			        					</div>			        					
			        			</li>
