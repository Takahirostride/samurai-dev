...
$(document).ready(function() {
	...
	//display current
	$.ajax({
		url: base_url + '/client/mypage/profile-ajax',
...
var loadCatAll = function(dataLoad, tHeadVal, checkAllId) {
	...
	let c = 0;
	html += '<thead>'+
					'<th colspan="2" class="text-center">'+tHeadVal+'</th>'+
	...
	$.each(dataLoad, function(index, val) {
		...
		html += '<tr>'+
						'<td width="40">Q</td>'+
						'<td>'+val+'</td>'+
						'<td class="text-center" width="200"><div class="checkbox"><label><input type="checkbox" class="'+checkAllId+' check-sub" name="subcat[]" '+selected+' value="'+index+'" id="check_list-'+index+'"></label></div></td>'+
					'</tr>';
	});
	html += '</tbody>';
...
var loadSubCat = function(json, catId) {
	html = '<h4 class="pagerow-title" id="sub-cat"><span>興味のある内容</span></h4>';
	for(i = 0; i < catId.length; i++) {
		html += '<table id="subcat-table-'+catId[i]+'" class="table table-bordered dtable table-condensed">';
